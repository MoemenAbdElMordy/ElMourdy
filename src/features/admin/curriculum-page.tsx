...
import { createContent, deleteContent, loadCurriculum, reorderContent, updateContent, type Branch, type Chapter, type ContentStatus, type Curriculum, type Lecture, type Lesson, type ResourceType, type VideoAssetSummary } from "../../shared/curriculum/api";
...
const emptyEditor={title:"",description:"",attachmentName:"",attachmentUrl:"",publishAt:"",isFree:false};
...
export function CurriculumManagePage(){
  ...
  const openEditor=(item?:ContentItem)=>{
    ...
    setEditor(item?{title:item.title,description:lecture?.description??"",attachmentName:lecture?.attachment_name??"",attachmentUrl:lecture?.attachment_url??"",publishAt:item.publish_at?.slice(0,16)??"",isFree:lecture?.is_free??false}:emptyEditor);
    setThumbnailFile(null);setRemoveThumbnail(false);setModal(true);
  };
  const save=async()=>{
    setSaving(true);
    try{
      const lectureFields=level==="lectures"?{description:editor.description||null,attachment_name:editor.attachmentName||null,attachment_url:editor.attachmentUrl||null,publish_at:editor.publishAt||null,is_free:editor.isFree}:{};
