...
export function CurriculumManagePage(){
  ...
  return <div className="min-h-screen bg-background p-4 sm:p-6"><div className="mx-auto max-w-5xl">
    ...
    <Modal2 open={modal} onClose={()=>setModal(false)} title={editingId?"تعديل المحتوى":"إضافة محتوى"} onSubmit={save}><div className="space-y-4"><Input2 label="العنوان" value={title} onChange={event=>setTitle(event.target.value)}/><Btn type="submit" className="w-full" disabled={!title.trim()}>{editingId?"حفظ التعديل":"حفظ كمسودة"}</Btn></div></Modal2>
