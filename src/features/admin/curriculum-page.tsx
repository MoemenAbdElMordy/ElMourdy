...
const levelLabel:Record<ResourceType,string>={branches:"مادة",chapters:"مجلد",lessons:"مجلد",lectures:"محاضرة"};
...
export function CurriculumManagePage({ params }: any){
  ...
  return <div className="min-h-screen bg-background p-4 sm:p-6"><div className="mx-auto max-w-6xl">
    <div className="mb-5 flex items-center justify-between gap-3"><div>{selection.branch&&<button onClick={back} className="mb-2 flex items-center gap-1 text-sm text-muted-foreground"><ChevronLeft size={14}/> رجوع</button>}<h1 className="text-2xl font-black">{heading}</h1><p className="mt-1 text-sm text-muted-foreground">رتّب المحتوى داخل مجلدات أو أضف المحاضرة مباشرة</p></div><div className="flex flex-wrap justify-end gap-2">{(level==="chapters"||level==="lessons")&&<Btn disabled={saving} onClick={openDirectLectureEditor}><Plus size={15}/> إضافة محاضرة</Btn>}<Btn variant={(level==="chapters"||level==="lessons")?"outline":undefined} onClick={()=>openEditor()}><Plus size={15}/> إضافة {levelLabel[level]}</Btn></div></div>
