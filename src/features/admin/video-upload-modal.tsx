...
export function VideoUploadModal({lecture,existingAsset,onClose,onReady}:{lecture:{id:number;title:string;video_source_type?:"uploaded"|"youtube";youtube_video_id?:string|null};existingAsset?:VideoAssetSummary|null;onClose:()=>void;onReady:()=>void}){
  ...
  useEffect(()=>{if(!existingAsset)return;loadVideoAsset(existingAsset.id).then(response=>{setAsset(response.video_asset);setStatusRefreshFailed(false);}).catch(()=>setStatusRefreshFailed(true));},[existingAsset?.id]);
  useEffect(()=>{loadReusableVideoAssets().then(response=>{setReusable(response.video_assets);if(!existingAsset&&lecture.video_source_type!=="youtube"){const latest=response.video_assets.find(item=>item.lecture_id===lecture.id&&item.used_by_lectures_count);if(latest){setAsset(latest);setMode(current=>current==="upload"&&!inputRef.current?.files?.length?"manage":current);}}}).catch(()=>setReusable([]));},[existingAsset?.id,lecture.id,lecture.video_source_type]);
  useEffect(()=>{
    ...
    let cancelled=false;
    const timer=window.setInterval(async()=>{try{const response=await loadVideoAsset(asset.id);if(cancelled)return;setAsset(response.video_asset);setStatusRefreshFailed(false);if(response.video_asset.processing_status==="ready"&&allQualitiesReady(response.video_asset)){setBusy(false);onReady();notify("الفيديو جاهز بجميع الجودات","success");}if(response.video_asset.processing_status==="failed")setBusy(false);}catch{if(!cancelled)setStatusRefreshFailed(true);}},5000);
    return()=>{cancelled=true;window.clearInterval(timer);};
  },[asset?.id,asset?.processing_status,onReady]);

  const start=async()=>{if(!file)return;setBusy(true);setProgress(0);try{const created=await createVideoUpload(lecture.id,file);setAsset(created.video_asset);await uploadVideoFile(file,created.upload,setProgress);setAsset((await completeVideoUpload(lecture.id,created.video_asset.id)).video_asset);setMode("manage");setBusy(false);onReady();notify("تم رفع الفيديو وبدأت المعالجة","success");}catch(error){notify(error instanceof ApiError?error.message:error instanceof Error?error.message:"تعذر رفع الفيديو","error");setBusy(false);}};
  const refreshStatus=async()=>{if(!asset)return;setBusy(true);try{const response=await loadVideoAsset(asset.id);setAsset(response.video_asset);setStatusRefreshFailed(false);onReady();}catch{setStatusRefreshFailed(true);notify("تعذر تحديث الحالة الآن؛ سنحاول تلقائيًا مرة أخرى","error");}finally{setBusy(false);}};
  ...
  return <Modal2 open onClose={onClose} title="إدارة فيديو المحاضرة" size="lg"><div className="space-y-4">
    ...
    {mode==="manage"&&asset&&<>
      ...
      {status==="processing"||status==="uploaded"||partiallyReady?<div className="rounded-xl bg-primary/10 p-4 text-center"><RefreshCw className="mx-auto mb-2 animate-spin text-primary"/><p>تجهيز الجودات المتبقية مستمر في الخلفية، ويمكنك إغلاق النافذة بأمان.</p>{statusRefreshFailed&&<p className="mt-2 text-sm text-amber-700">تعذر تحديث الحالة مؤقتًا؛ سنحاول تلقائيًا مرة أخرى.</p>}<Btn className="mt-3" variant="outline" disabled={busy} onClick={refreshStatus}><RefreshCw size={15}/> تحديث الحالة الآن</Btn></div>:null}
    ...
    {mode==="reuse"&&<div className="space-y-3"><div><p className="mb-2 text-sm font-bold">اختر فيديو مرفوعًا من قبل</p><div role="radiogroup" aria-label="الفيديوهات المرفوعة سابقًا" className="max-h-72 space-y-2 overflow-y-auto">{reusable.filter(item=>item.processing_status==="ready").map(item=><button type="button" role="radio" aria-checked={String(item.id)===selectedAssetId} key={item.id} onClick={()=>setSelectedAssetId(String(item.id))} className={`flex w-full items-center justify-between gap-3 rounded-xl border p-3 text-right transition ${String(item.id)===selectedAssetId?"border-primary bg-primary/10 ring-2 ring-primary/20":"border-border hover:border-primary/60"}`}><span><strong>{item.lecture_title??`فيديو رقم ${item.id}`}</strong><small className="mt-1 block text-muted-foreground">المدة: {formatDuration(item.duration_seconds)} • الجودات: {item.available_qualities?.join("، ")||"جاهزة"}</small></span><span className={`h-5 w-5 shrink-0 rounded-full border-2 ${String(item.id)===selectedAssetId?"border-primary bg-primary shadow-[inset_0_0_0_4px_var(--background)]":"border-border"}`}/></button>)}</div>{!reusable.some(item=>item.processing_status==="ready")&&<p className="rounded-xl bg-muted p-4 text-center text-sm text-muted-foreground">لا توجد فيديوهات جاهزة لإعادة الاستخدام.</p>}</div><p className="text-xs text-muted-foreground">سيُستخدم نفس الملف والجودات من غير نسخ أو رفع أو معالجة جديدة.</p><Btn className="w-full" disabled={!selectedAssetId||busy} onClick={reuse}><Library size={15}/>{busy?"جارٍ ربط الفيديو…":selectedReusable?`تأكيد استخدام: ${selectedReusable.lecture_title??`فيديو رقم ${selectedReusable.id}`}`:"اختر فيديو أولًا"}</Btn></div>}
  </div></Modal2>;
}
