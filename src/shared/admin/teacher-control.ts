...
export const loadParents=(query="",status="")=>{const params=new URLSearchParams();if(query)params.set("query",query);if(status)params.set("status",status);return apiRequest<{parents:ParentRecord[]}>(`/parents?${params}`);};
...
export const loadManagementReport=(academicYearId?:number,gradeId?:number)=>{const params=new URLSearchParams();if(academicYearId)params.set("academic_year_id",String(academicYearId));if(gradeId)params.set("grade_id",String(gradeId));return apiRequest<{report:ManagementReport}>(`/management_report?${params}`);};
