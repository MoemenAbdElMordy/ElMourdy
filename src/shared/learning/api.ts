...
export const loadExams = (filters:{lessonId?:number;page?:number}={}) => {const params=new URLSearchParams();if(filters.lessonId)params.set("lesson_id",String(filters.lessonId));addPagination(params,filters.page);return apiRequest<{exams:Exam[];pagination:PaginationMeta}>(`/exams?${params}`);};
